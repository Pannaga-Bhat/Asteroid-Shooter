...
#include "main.cpp" // Replace this with your game header file
#include <iostream>
...
TEST(PlayerMovementTest, PlayerMovement) {
    ...
    float initial_dx = p->dx;
    float initial_dy = p->dy;

    // Simulate player movement update
    p->update();

    // Check if the player's movement updates correctly
    EXPECT_NE(p->dx, initial_dx);
    EXPECT_NE(p->dy, initial_dy);
